...
const hasDeviation = line.actualCost !== undefined && line.actualCost > 0 && line.subtotal;
  ...
  return (
    <>
      <tr className={`group hover:bg-blue-50/30 transition-colors ${hasDeviation ? 'bg-amber-50/20' : ''}`}>
        {/* Renglon */}
...
{editingAllowed ? (
        ...
        {/* TOTAL MATERIAL */}
        <td className="text-[9px] font-bold text-right text-slate-800 bg-slate-50/50 tabular-nums">
        ...
        {/* SUBTOTAL */}
        <td className="text-[10px] font-black text-right text-blue-700 bg-blue-50/30 tabular-nums border-l border-slate-200">
...
{editingAllowed ? (
        ...
        <td className="text-center">
          {r?.steelInfo && (
            <span className="text-[8px] text-orange-600 font-bold cursor-pointer" onClick={() => onSteelClick?.(line, r.steelInfo! as ReturnType<typeof calcSteelReinforcement>)}
...
{line.computationType === 'dynamic' && (
      ...
      {editingAllowed && line.computationType === 'dynamic' && (
        <tr key={`${line.id}_dim`} className="bg-blue-50 border-t border-blue-200">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
        <h3 className="text-sm font-black text-slate-800 uppercase mb-4">Acero de Refuerzo — {line.description}</h3>
        <div className="grid grid-cols-2 gap-3 text-[10px]">
          {[
            ...
            <div key={label} className="bg-slate-50 rounded-lg p-2">
              <div className="text-slate-500 text-[8px] uppercase">{label}</div>
              <div className="text-slate-800 font-bold">{value}</div>
            </div>
          ))}
        </div>
        <div className="mt-4 bg-orange-50 rounded-lg p-3 border border-orange-200">
          <div className="text-[9px] font-bold text-orange-700 mb-1">Densidad del acero</div>
          <div className="text-[10px] text-slate-600">{ENGINEERING.densities.steel} kg/m³ — Ratio aplicado según tipología</div>
        </div>
        <button onClick={onClose} className="mt-4 w-full px-4 py-2 text-[10px] font-bold uppercase bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200">Cerrar</button>
      </div>
    </div>
  );
});
...
export default function BudgetTable({
  ...
  return (
    <div className="overflow-x-auto">
      {/* Resumen de totales */}
      <div className="grid grid-cols-6 gap-2 mb-4 p-3 bg-slate-50 rounded-lg">
        {[
          ['Materiales', t.materialTotal, 'text-blue-700'],
          ['Mano Obra', t.laborTotal, 'text-green-700'],
          ['Equipo', t.equipmentTotal, 'text-slate-700'],
          ['Desperdicio', t.wasteTotal, 'text-orange-600'],
          ['IVA + Margen + Imprev.', PMath.sum([t.taxTotal, t.profitTotal, t.contingencyTotal]), 'text-purple-700'],
          ['TOTAL PRESUPUESTO', t.totalBudget, 'text-blue-800 font-black'],
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-slate-300 bg-slate-100">
            {['Renglón', 'Un', 'Cant', 'CMat', 'CMO', 'CEq', 'TotMat', 'TotMO', 'TotEq', 'Subtotal', 'Días', 'Rend%', 'Desp%', 'Mdo%', 'Acero'].concat(showDeviations ? ['Desv'] : []).concat(editingAllowed ? ['Acc'] : []).map((h, i) => (
              <th key={i} className="px-2 py-1.5 text-[8px] font-black text-slate-600 uppercase tracking-wider text-right first:text-left">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          ...
          <tr className="border-t-2 border-slate-400 bg-slate-100 font-black">
            <td className="px-4 py-2 text-[10px] text-slate-800 uppercase tracking-wider" colSpan={3}>
              TOTALES GENERALES
            </td>
            <td colSpan={3} />
            <td className="px-4 py-2 text-[10px] text-right text-slate-800">{fmtQ(t.materialTotal)}</td>
            <td className="px-4 py-2 text-[10px] text-right text-slate-800">{fmtQ(t.laborTotal)}</td>
            <td className="px-4 py-2 text-[10px] text-right text-slate-800">{fmtQ(t.equipmentTotal)}</td>
            <td className="px-4 py-2 text-[10px] text-right text-blue-800">{fmtQ(t.directCost)}</td>
            <td colSpan={5} />
          </tr>
        </tbody>
      </table>
