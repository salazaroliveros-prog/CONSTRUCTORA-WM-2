...
const InlineEdit = memo(function InlineEdit({
  ...
  useEffect(() => { if (editing && ref.current) { ref.current.focus(); ref.current.select(); } }, [editing]);
    ...
    if (disabled) return <span className="text-[9px] text-(--color-neutral-400)">{display}</span>;
if (editing) {
     return (
       <div className="inline-flex items-center gap-0.5">
         ...
         <input id={`inline-input-${id}`} ref={ref} type="text" inputMode="decimal" aria-label={label || 'Editar valor'}
           ...
           className="w-20 text-[9px] text-right px-1 py-0.5 border border-[color-mix(in_srgb,var(--color-info)_40%,transparent)] rounded bg-(--color-surface-solid) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--color-info) font-mono tabular-nums"
         />
<button aria-label="Confirmar" onClick={() => { const n = parseQtyInput(draft); if (validateQty(n) || n === 0) onCommit(n); setEditing(false); }} className="p-0.5 text-(--color-success) hover:text-[color-mix(in_srgb,var(--color-success)_80%,black)]"><Check size={10} /></button>
          <button aria-label="Cancelar" onClick={() => setEditing(false)} className="p-0.5 text-(--color-error) hover:text-[color-mix(in_srgb,var(--color-error)_80%,black)]"><X size={10} /></button>
       </div>
     );
   }
  return (
     <span className="text-[9px] text-right cursor-pointer hover:bg-[color-mix(in_srgb,var(--color-warning)_10%,transparent)] px-1 rounded text-(--color-neutral-600) hover:text-(--color-neutral-900) tabular-nums"
      onClick={() => {
...
{hasChildren ? (
                <button onClick={() => onToggle(line.id)} aria-label={`Expandir renglón ${line.code}`} className="w-5 h-5 flex items-center justify-center rounded hover:bg-(--color-neutral-200) transition-colors">
                 {isExpanded ? <ChevronDown size={12} aria-hidden="true" /> : <ChevronRight size={12} aria-hidden="true" />}
               </button>
             ) : <span className="w-5" />}

{line.computationType === 'dynamic' && (
                <span className="text-[8px] text-(--color-info) font-bold" aria-label="Cálculo dinámico por dimensiones"><GitBranch size={9} /></span>
             )}
             {r?.computationType === 'steel' && (
                <span className="text-[8px] text-(--color-warning) font-bold" aria-label="Acero de refuerzo"><Zap size={9} /></span>
             )}

             <span className="text-[9px] font-black text-(--color-neutral-700) uppercase">{line.code}</span>
             <span className="text-[9px] font-bold text-(--color-neutral-600) truncate max-w-45" title={line.description}>{line.description}</span>

            {hasDeviation && (
              <span title={`Desviación: ${(((line.actualCost! - (line.subtotal || 0)) / (line.subtotal || 1)) * 100).toFixed(1)}%`}>
                 <AlertTriangle size={9} className="text-(--color-warning) shrink-0" />
              </span>
            )}
          </div>
        </td>
        ...
        {/* Unidad */}
        <td className="text-[9px] text-(--color-neutral-500) text-right font-mono">{line.unit}</td>

        {/* Cantidad */}
{line.computationType === 'dynamic' ? (
           <td className="text-right">
              <button onClick={() => onEdit(line.id)} className="text-[9px] text-(--color-info) font-bold hover:bg-[color-mix(in_srgb,var(--color-info)_15%,transparent)] px-1 rounded" aria-label={`Editar dimensiones de ${line.description}`}>
           ...
           <td className="text-right">
             {editingAllowed ? (
               <InlineEdit id={`qty-${line.id}`} label={`Cantidad ${line.code}`} value={r?.qty ?? line.qty} display={fmtQty(r?.qty ?? line.qty, 0, 6)} onCommit={v => commit('qty', v)} />
                ...
                <span className="text-[9px] text-(--color-neutral-600) tabular-nums">{fmtQty(r?.qty ?? line.qty, 0, 6)}</span>
             )}
           </td>
         )}
...
{editingAllowed ? (
             <InlineEdit id={`mat-${line.id}`} label={`Costo material ${line.code}`} value={r?.materialCost ?? line.materialCost} display={fmtMoney(r?.materialCost ?? line.materialCost)} onCommit={v => commit('materialCost', v)} />
              ...
              <span className="text-[9px] text-(--color-neutral-500) tabular-nums">{fmtMoney(r?.materialCost ?? line.materialCost)}</span>
           )}
        </td>
...
{editingAllowed ? (
             <InlineEdit id={`lab-${line.id}`} label={`Costo mano de obra ${line.code}`} value={r?.laborCost ?? line.laborCost} display={fmtMoney(r?.laborCost ?? line.laborCost)} onCommit={v => commit('laborCost', v)} />
              ...
              <span className="text-[9px] text-(--color-neutral-500) tabular-nums">{fmtMoney(r?.laborCost ?? line.laborCost)}</span>
           )}
        </td>
...
{editingAllowed ? (
             <InlineEdit id={`eq-${line.id}`} label={`Costo equipo ${line.code}`} value={line.equipmentCost ?? 0} display={fmtMoney((line.equipmentCost ?? 0))} onCommit={v => commit('equipmentCost', v)} />
              ...
              <span className="text-[9px] text-(--color-neutral-500) tabular-nums">{fmtMoney((line.equipmentCost ?? 0))}</span>
           )}
        </td>

        {/* TOTAL MATERIAL */}
        <td className="text-[9px] font-bold text-right text-(--color-neutral-800) bg-(--color-neutral-50)/50 tabular-nums">
         ...
         {/* SUBTOTAL */}
         <td className="text-[10px] font-black text-right text-(--color-info) bg-[color-mix(in_srgb,var(--color-info)_10%,transparent)]/30 tabular-nums border-l border-(--color-neutral-200)">
...
{editingAllowed ? (
             <InlineEdit id={`dur-${line.id}`} label={`Duración días ${line.code}`} value={line.durationDays ?? 0} display={line.durationDays ? `${line.durationDays}d` : '—'} onCommit={v => commit('durationDays', v)} />
              ...
              <span className="text-[9px] text-(--color-neutral-500)">{line.durationDays ? `${line.durationDays}d` : '—'}</span>
           )}
        </td>
...
{editingAllowed ? (
             <InlineEdit id={`perf-${line.id}`} label={`Rendimiento ${line.code}`} value={line.laborPerf ?? 1} display={line.laborPerf && line.laborPerf !== 1 ? fmtPct(line.laborPerf) : '—'} type="percent" onCommit={v => commit('laborPerf', v / 100)} />
              ...
              <span className="text-[9px] text-(--color-neutral-500)">{line.laborPerf && line.laborPerf !== 1 ? fmtPct(line.laborPerf) : '—'}</span>
           )}
        </td>
...
{editingAllowed ? (
             <InlineEdit id={`waste-${line.id}`} label={`Desperdicio ${line.code}`} value={line.wasteFactor ?? 1} display={line.wasteFactor && line.wasteFactor !== 1 ? `${((line.wasteFactor - 1) * 100).toFixed(0)}%` : '—'} type="percent" onCommit={v => commit('wasteFactor', v / 100)} />
               ...
               <span className="text-[9px] text-(--color-neutral-500)">{line.wasteFactor && line.wasteFactor !== 1 ? `${((line.wasteFactor - 1) * 100).toFixed(0)}%` : '—'}</span>
           )}
        </td>
...
{editingAllowed ? (
             <InlineEdit id={`mkt-${line.id}`} label={`Índice mercado ${line.code}`} value={line.marketPriceIndex ?? 1} display={line.marketPriceIndex && line.marketPriceIndex !== 1 ? fmtPct(line.marketPriceIndex) : '—'} type="percent" onCommit={v => commit('marketPriceIndex', v / 100)} />
              ...
              <span className="text-[9px] text-(--color-neutral-500)">{line.marketPriceIndex && line.marketPriceIndex !== 1 ? fmtPct(line.marketPriceIndex) : '—'}</span>
           )}
        </td>
        ...
        <td className="text-center">
          {r?.steelInfo && (
             <span className="text-[8px] text-(--color-warning) font-bold cursor-pointer" onClick={() => onSteelClick?.(line, r.steelInfo! as ReturnType<typeof calcSteelReinforcement>)}
...
{hasDeviation && (
           <td className="text-[9px] text-right">
              <span className={`font-bold ${deviationPct > 0 ? 'text-(--color-error)' : 'text-(--color-success)'}`} aria-label={`Desviación ${deviationPct.toFixed(1)}%`}>
               {fmtDev(deviationPct)}
             </span>
           </td>
         )}
        ...
        {editingAllowed && (
          <td className="text-right">
{line.computationType === 'dynamic' && (
              <button onClick={() => onEdit(line.id)} aria-label={`Editar dimensiones de ${line.code}`} className="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-(--color-neutral-200) transition-all" title="Editar dimensiones"><Pencil size={11} /></button>
           )}
           <button onClick={() => {
            ...
            }} aria-label={`Eliminar ${line.description}`} className="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-[color-mix(in_srgb,var(--color-error)_15%,transparent)] text-(--color-error) transition-all" title="Eliminar"><Trash2 size={11} /></button>
          </td>
        )}
      </tr>
...
const SteelModal = memo(function SteelModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-(--color-surface-solid) rounded-xl shadow-2xl p-6 max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
        <h3 className="text-sm font-black text-(--color-neutral-800) uppercase mb-4">Acero de Refuerzo — {line.description}</h3>
        <div className="grid grid-cols-2 gap-3 text-[10px]">
          {[
            ...
            <div key={label} className="bg-(--color-neutral-50) rounded-lg p-2">
              <div className="text-(--color-neutral-500) text-[8px] uppercase">{label}</div>
              <div className="text-(--color-neutral-800) font-bold">{value}</div>
            </div>
          ))}
        </div>
        <div className="mt-4 bg-[color-mix(in_srgb,var(--color-warning)_10%,transparent)] rounded-lg p-3 border border-[color-mix(in_srgb,var(--color-warning)_20%,transparent)]">
          <div className="text-[9px] font-bold text-(--color-warning) mb-1">Densidad del acero</div>
          <div className="text-[10px] text-(--color-neutral-600)">{ENGINEERING.densities.steel} kg/m³ — Ratio aplicado según tipología</div>
        </div>
        <button onClick={onClose} className="mt-4 w-full px-4 py-2 text-[10px] font-bold uppercase bg-(--color-neutral-100) text-(--color-neutral-700) rounded-lg hover:bg-(--color-neutral-200)">Cerrar</button>
      </div>
    </div>
  );
});
...
export default function BudgetTable({
  ...
  return (
    <div className="overflow-x-auto">
      {/* Resumen de totales */}
      <div className="grid grid-cols-6 gap-2 mb-4 p-3 bg-(--color-neutral-50) rounded-lg">
        {[
          ['Materiales', t.materialTotal, 'text-(--color-info)'],
          ['Mano Obra', t.laborTotal, 'text-(--color-success)'],
          ['Equipo', t.equipmentTotal, 'text-(--color-neutral-700)'],
          ['Desperdicio', t.wasteTotal, 'text-(--color-warning)'],
          ['IVA + Margen + Imprev.', PMath.sum([t.taxTotal, t.profitTotal, t.contingencyTotal]), 'text-(--color-secondary-dark)'],
          ['TOTAL PRESUPUESTO', t.totalBudget, 'text-[color-mix(in_srgb,var(--color-info)_80%,black)] font-black'],
        ].map(([label, value, color]) => (
          <div key={label} className="text-center">
            <div className="text-[8px] text-(--color-neutral-500) uppercase">{label}</div>
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-(--color-neutral-300) bg-(--color-neutral-100)">
            {['Renglón', 'Un', 'Cant', 'CMat', 'CMO', 'CEq', 'TotMat', 'TotMO', 'TotEq', 'Subtotal', 'Días', 'Rend%', 'Desp%', 'Mdo%', 'Acero'].concat(showDeviations ? ['Desv'] : []).concat(editingAllowed ? ['Acc'] : []).map((h, i) => (
              <th key={i} className="px-2 py-1.5 text-[8px] font-black text-(--color-neutral-600) uppercase tracking-wider text-right first:text-left">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          ...
          <tr className="border-t-2 border-(--color-neutral-400) bg-(--color-neutral-100) font-black">
            <td className="px-4 py-2 text-[10px] text-(--color-neutral-800) uppercase tracking-wider" colSpan={3}>
              TOTALES GENERALES
            </td>
            <td colSpan={3} />
            <td className="px-4 py-2 text-[10px] text-right text-(--color-neutral-800)">{fmtQ(t.materialTotal)}</td>
            <td className="px-4 py-2 text-[10px] text-right text-(--color-neutral-800)">{fmtQ(t.laborTotal)}</td>
            <td className="px-4 py-2 text-[10px] text-right text-(--color-neutral-800)">{fmtQ(t.equipmentTotal)}</td>
      ...
      {editingAllowed && (
        <div className="p-4 border-t border-(--color-neutral-200)">
          <button onClick={onAddCustom}
            className="flex items-center gap-2 px-4 py-2 text-[9px] font-bold uppercase bg-(--color-neutral-100) text-(--color-neutral-700) rounded-lg hover:bg-(--color-neutral-200) transition-colors">
