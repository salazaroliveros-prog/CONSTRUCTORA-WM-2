  ...
  return (
    <div id="warehouse-management" className="space-y-6 animate-in fade-in duration-500 pb-[calc(4rem+env(safe-area-inset-bottom,0px))] scroll-mb-[calc(4rem+env(safe-area-inset-bottom,0px))]">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-neutral-900/40 p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-3 text-left">
        ...
        {[{ id: 'stock', label: 'Stock / Bodega', icon: <Package size={13}/> }, { id: 'orders', label: 'Ordenes de Compra', icon: <ShoppingCart size={13}/> }].map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id as any)}
            className={cn('flex items-center gap-2 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all',
              activeTab === t.id ? 'bg-slate-900 text-white shadow' : 'bg-neutral-900/40 border border-slate-200 text-slate-500 hover:border-secondary')}>
            {t.icon}{t.label}
          </button>
        ))}
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest ml-1">Proyecto</span>
           <select value={filterProject} onChange={e => setFilterProject(e.target.value)}
             ...
             className="px-3 py-2 bg-neutral-900/40 border border-slate-200 rounded-xl text-[9px] font-black uppercase focus:outline-none focus:border-secondary min-w-40">
      ...
      <div className="hidden lg:block bg-neutral-900/40 p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-12 gap-6 items-center">
          <div className="col-span-7 bg-black/20 rounded-xl p-4 border border-slate-100 flex items-center justify-center min-h-[200px]">
                ...
                <motion.div 
                  ...
                  className={cn(
                    ...
                    selectedZone === zone.id 
                      ...
                      : "bg-neutral-900/40 border-slate-100 shadow-sm text-slate-400 hover:border-secondary"
                  )}
                >
               ...
               return (
                 <div 
                   ...
                   className={cn(
                     ...
                     isActive ? "bg-slate-900 border-slate-900" : "bg-neutral-900/40 border-slate-100 hover:bg-black/20"
                   )}
                 >
      ...
      <div className="bg-neutral-900/40 rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {categories.map((cat) => (
              <button 
                ...
                className={cn(
                  ...
                  activeCategory === cat ? "bg-slate-900 text-white border-slate-900 shadow-md" : "bg-neutral-900/40 text-slate-500 border-slate-200 hover:bg-black/20 shadow-sm"
                )}
              >
                {cat}
              </button>
        ...
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-black/20/50 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">
                {bulkMode && (
            ...
            <tbody className="divide-y divide-slate-50">
              {paginatedItems.map((item, i) => (
                  <motion.tr
                    ...
                    className={cn(
                      "hover:bg-black/20/50 transition-all cursor-pointer group",
                      selectedItem === item.id || selectedItemIds.has(item.id) ? "bg-black/20/80" : ""
                    )}
      ...
      {filterProject !== 'ALL' && filterProject !== 'GENERAL' && (() => {
        ...
        return (
          <div className="bg-neutral-900/40 rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-neutral-900/40 rounded-2xl shadow-modal overflow-hidden border border-p-200"
            >
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <div className="flex bg-black/20 p-0.5 rounded-lg border border-p-200">
                      <button type="button"
                        onClick={() => setMovementForm({ ...movementForm, type: 'Entrada', category: entryCategories[0] })}
                        className={cn("flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all", movementForm.type === 'Entrada' ? "bg-neutral-900/40 text-green-600 shadow-sm" : "text-p-400")}>
                        Entrada
                      </button>
                      <button type="button"
                        onClick={() => setMovementForm({ ...movementForm, type: 'Salida', category: exitCategories[0] })}
                        className={cn("flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all", movementForm.type === 'Salida' ? "bg-neutral-900/40 text-red-600 shadow-sm" : "text-p-400")}>
                        Salida
                      </button>
                    </div>
                  </div>
      ...
      {activeTab === 'orders' && (
        <div className="space-y-4">
          {purchaseOrders.length === 0 ? (
            <div className="bg-neutral-900/40 border border-slate-100 rounded-2xl p-10 text-center">
            ...
            <div key={order.id} className="bg-neutral-900/40 border border-slate-100 rounded-2xl p-4 shadow-sm relative">
              {bulkMode && (
              ...
              <div>
                ...
                <div className="max-h-32 overflow-y-auto space-y-1 border border-slate-100 rounded-xl p-2">
                  {(item?.materials || []).map((m: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between bg-black/20 rounded-lg px-2 py-1">
      ...
      {bulkMode && activeTab === 'stock' && selectedItemIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-red-600 text-white px-5 py-3 rounded-2xl shadow-modal flex items-center gap-4">
          ...
          <button type="button" onClick={handleBulkDeleteItems} className="px-4 py-1.5 bg-neutral-900/40 text-red-600 rounded-xl text-[8px] font-black uppercase tracking-widest hover:bg-red-50 transition-all">Eliminar</button>
      ...
      {bulkMode && activeTab === 'orders' && selectedOrderIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-red-600 text-white px-5 py-3 rounded-2xl shadow-modal flex items-center gap-4">
          ...
          <button type="button" onClick={handleBulkDeleteOrders} className="px-4 py-1.5 bg-neutral-900/40 text-red-600 rounded-xl text-[8px] font-black uppercase tracking-widest hover:bg-red-50 transition-all">Eliminar</button>
