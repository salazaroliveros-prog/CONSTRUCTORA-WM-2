...
const ROLE_COLORS: Record<string, { bg: string; text: string; dot: string; border: string }> = {
  'MAESTRO DE OBRA': { bg: 'bg-(--color-warning-bg)',  text: 'text-warning',  dot: 'bg-accent',  border: 'border-(--color-amber-border)' },
  'ALBAÑIL':         { bg: 'bg-(--color-info-bg)',    text: 'text-info',   dot: 'bg-info',   border: 'border-(--color-blue-border)' },
  ...
  'PLANIFICADOR':    { bg: 'bg-(--color-success-bg)',   text: 'text-success',  dot: 'bg-success',  border: 'border-(--color-green-border)' },
  ...
  'SUPERVISOR':      { bg: 'bg-(--color-error-bg)',     text: 'text-error',    dot: 'bg-error',    border: 'border-red-300' },
};
...
{[
          { label: 'Total Personal', value: staff.length, icon: <Users size={12}/>, color: 'text-info', bg: 'bg-(--color-info-bg)' },
          { label: 'Activos', value: activeCount, icon: <UserCheck size={12}/>, color: 'text-success', bg: 'bg-(--color-success-bg)' },
          { label: 'Asignados', value: assignedCount, icon: <Building2 size={12}/>, color: 'text-(--color-mod-dashboard)', bg: 'bg-(--color-mod-dashboard)/10' },
          { label: 'Nómina Total', value: `Q ${totalSalary.toLocaleString('es-GT')}`, icon: <DollarSign size={12}/>, color: 'text-warning', bg: 'bg-(--color-warning-bg)' },
        ].map((kpi, i) => (
...
<div className="relative flex-1 md:w-48">
            ...
            <>
              ...
              <button type="button" onClick={() => setIsPayrollModalOpen(true)}
                className="flex items-center gap-1.5 px-3 py-2 bg-success text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-success transition-all shrink-0">
                      ...
                      <div className="mt-2.5 pt-2.5 border-t border-slate-50 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                          {assignedToProjects.length > 0 && (
                            <span className="text-[7px] font-black bg-(--color-info-bg) text-info px-1.5 py-0.5 rounded-full">{assignedToProjects.length} proy.</span>
                          )}
                          <span className={cn("text-[7px] font-black uppercase px-1.5 py-0.5 rounded-full",
                            m.status === 'Activo' || !m.status ? "bg-(--color-success-bg) text-success" : "bg-(--color-error-bg) text-error")}>
                            {m.status || 'Activo'}
                          </span>
                        </div>
                      </div>
...
<input type="checkbox" checked={currentItems.length > 0 && selectedPersonalIds.size === currentItems.length}
                                onChange={toggleSelectAllPersonal} title="Seleccionar todo" className="w-3.5 h-3.5 accent-error cursor-pointer" />
                           </th>
                         )}
...
{bulkMode && (
                             ...
                             <td className="hidden md:table-cell px-3 py-2">
                               {assignedToProjects.length > 0
                                 ? <span className="text-[7px] font-black bg-(--color-info-bg) text-info px-1 py-0.5 rounded-full">{assignedToProjects.length} proy{assignedToProjects.length > 1 ? 's' : ''}</span>
...
{bulkMode && selectedPersonalIds.size > 0 && activeStaffTab === 'personal' && (
         <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-error text-white px-4 py-2.5 rounded-xl shadow-2xl flex items-center gap-3">
           ...
           <button type="button" onClick={handleBulkDeletePersonal}
             className="px-3 py-1 bg-white text-error rounded-lg text-[7px] font-black uppercase tracking-widest hover:bg-(--color-error-bg) transition-all">
             Eliminar
           </button>
                 ...
                 <div className="flex items-center gap-2">
                   ...
                   <div className="min-w-0">
                     ...
                     <div className="flex items-center gap-1 mt-0.5">
                       ...
                       <span className={cn("text-[6px] font-black uppercase px-1 py-0.5 rounded-full",
                         (selectedMember.status || 'Activo') === 'Activo' ? "bg-success/20 text-success" : "bg-error/20 text-red-300")}>
               ...
               <div className="flex-1 overflow-y-auto p-3 space-y-3">
                 <div className="grid grid-cols-2 gap-1.5">
                   <div className="bg-(--color-warning-bg) rounded-md p-1.5 text-center">
                     <p className="text-[6px] font-black text-warning uppercase tracking-widest">Salario</p>
                      <p className="text-[10px] font-black text-warning">{fmtQ(Number(selectedMember.salary||0))}</p>
                   </div>
                   <div className="bg-(--color-info-bg) rounded-md p-1.5 text-center">
                     <p className="text-[6px] font-black text-info uppercase tracking-widest">Proyectos</p>
                     <p className="text-[10px] font-black text-info">{memberProjects.length}</p>
                   </div>
                 </div>
                <div>
                       ...
                       {memberProjects.map(p => (
                         <div key={p.id} className="bg-white/40 backdrop-blur-sm rounded-md p-1.5 flex items-center justify-between hover:bg-white/40 transition-all group">
                           <div className="flex-1 min-w-0">
                             <div className="flex items-center gap-1 mb-0.5">
                               ...
                               <span className={cn("text-[6px] font-black uppercase px-1 py-0.5 rounded-full",
                                 ...
                                 p.status === 'COTIZACION' ? "bg-(--color-info-bg) text-info" :
                           ...
                           <button type="button" title="Remover del proyecto"
                             ...
                             className="ml-1 p-0.5 rounded text-error hover:bg-(--color-error-bg) hover:text-error transition-all shrink-0 opacity-0 group-hover:opacity-100">
                             <X size={8} />
                           </button>
                         </div>
                       ))}
                   ...
                   <div className="space-y-0.5">
                     {projects.filter(p => p.status === 'EJECUCION' && !p.teamIds?.includes(selectedMember.id)).slice(0, 5).map(p => (
                       <button type="button" key={p.id} onClick={() => handleToggleProject(selectedMember.id, p.id)}
                         ...
                         <Plus size={8} className="text-slate-400 group-hover:text-info shrink-0" />
                       </button>
                     ))}
                             ...
                             <div key={p.id} className="bg-white/40 backdrop-blur-sm rounded-md p-1.5">
                               <div className="flex justify-between items-center">
                                 <div className="flex items-center gap-1">
                                   ...
                                   <span className={cn("text-[6px] font-black uppercase px-1 py-0.5 rounded-full",
                                     p.status === 'PAGADA' ? 'bg-(--color-success-bg) text-success' : 'bg-(--color-warning-bg) text-warning')}>
                                     {p.status}
                                   </span>
                                 </div>
      ...
      <div className="w-full overflow-y-auto space-y-3">
            {payrolls.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 opacity-40">
                ...
                <button type="button" onClick={() => setIsPayrollModalOpen(true)}
                  className="mt-4 text-[9px] font-black text-success border-b border-emerald-600 pb-0.5 uppercase tracking-widest">
              ...
              <div className="grid gap-2">
                {bulkMode && payrolls.length > 0 && (
                  <div className="flex items-center gap-2 px-1 py-1">
                     <input type="checkbox" checked={selectedPayrollIds.size === payrolls.length && payrolls.length > 0}
                       onChange={toggleSelectAllPayroll} title="Seleccionar todas las planillas" className="w-4 h-4 accent-error cursor-pointer" />
                ...
                {payrolls.slice().sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(pay => {
                  const statusColor = pay.status === 'PAGADA' ? 'bg-(--color-success-bg) text-success' : pay.status === 'BORRADOR' ? 'bg-(--color-warning-bg) text-warning' : 'bg-(--color-error-bg) text-error';
                  return (
                    <div key={pay.id}
                              ...
                              {pay.status === 'BORRADOR' && (
                                <>
                                  <button type="button" onClick={(e) => { e.stopPropagation(); openEditPayroll(pay); }}
                                    className="px-2 py-1 bg-(--color-info-bg) text-info rounded-lg text-[8px] font-black uppercase hover:bg-blue-200 transition-all">
                                    <Pencil size={10} className="inline mr-0.5" /> Editar
                                  </button>
                                  <button type="button" onClick={() => markPayrollAsPaid(pay)}
                                    className="px-2 py-1 bg-(--color-success-bg) text-success rounded-lg text-[8px] font-black uppercase hover:bg-emerald-200 transition-all">
                                    Marcar Pagada
                                  </button>
                                </>
                              )}
                              <button type="button" onClick={(e) => { e.stopPropagation(); handleDeletePayroll(pay.id); }} aria-label="Eliminar planilla"
                                className="px-2 py-1 bg-(--color-error-bg) text-error rounded-lg text-[8px] font-black uppercase hover:bg-red-200 transition-all">
             ...
             <div>
               <div className="flex justify-between items-center mb-1">
                 ...
                 <button type="button" onClick={generatePayrollPreview}
                   className="text-[7px] font-black text-info uppercase hover:text-info">
...
<textarea value={payrollForm.notes} onChange={e => setPayrollForm(f => ({ ...f, notes: e.target.value }))} title="Notas"
           ...
           <div className="flex justify-between items-center pt-1 border-t border-slate-100 text-[9px]">
             ...
             <button type="button" onClick={createPayroll} disabled={savingPayroll || payrollForm.employees.length === 0}
               className="px-3 py-1.5 bg-success text-white rounded-lg text-[8px] font-black uppercase tracking-widest hover:bg-success disabled:opacity-50 transition-all">
...
<textarea value={editingPayrollForm.notes} onChange={e => setEditingPayrollForm(f => ({ ...f, notes: e.target.value }))} title="Notas"
           ...
           <div className="flex justify-between items-center pt-1 border-t border-slate-100 text-[9px]">
             ...
             <button type="button" onClick={saveEditPayroll} disabled={savingEditPayroll}
               className="px-3 py-1.5 bg-info text-white rounded-lg text-[8px] font-black uppercase tracking-widest hover:bg-info disabled:opacity-50 transition-all">
