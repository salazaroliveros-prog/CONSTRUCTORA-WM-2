  ...
  >
    ...
    <div className="p-4 space-y-3 flex-1">
      <div className="flex justify-between items-start">
        <div className="w-10 h-10 rounded-xl bg-neutral-900 border border-slate-800 flex items-center justify-center text-secondary group-hover:scale-105 transition-transform duration-300 icon-box icon-gradient-blue">
          <Building2 size={20} />
        </div>
        <div className="flex flex-col items-end sm:items-end">
          <span className={cn(
            ...
            project.status === 'COTIZACION' ? "bg-info text-neutral-50" :
            "bg-success text-neutral-50"
          )}>
            {project.status}
          </span>
          <span className="text-[7px] sm:text-[6px] font-bold text-neutral-400 mt-1 uppercase tracking-tighter">Cód: {project.id.slice(-6).toUpperCase()}</span>
        </div>
      </div>
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-[8px] font-bold text-neutral-400 uppercase tracking-widest truncate">{project.clientName}</p>
      </div>

      <div className="pt-2 space-y-1.5 border-t border-slate-50">
        <div className="flex justify-between items-end">
          <span className="text-[7px] font-black text-neutral-400 uppercase tracking-widest">Progreso</span>
          <div className="flex items-center gap-1">
            {project.status === 'EJECUCION' && (new Date().getTime() - new Date(project.startDate).getTime()) / (new Date(project.endDate || project.startDate).getTime() - new Date(project.startDate).getTime() || 1) > 0.1 && (project.progress || 0) < 10 && (
              <span title="Progreso atrasado respecto al tiempo transcurrido"><AlertCircle size={10} className="text-error animate-pulse" /></span>
            )}
            <span className="text-[9px] font-black text-primary">{project.progress || 0}%</span>
            <div className="w-1 h-1 rounded-full bg-secondary animate-pulse" />
          </div>
        </div>
        <div className="h-1.5 bg-neutral-100 rounded-full overflow-hidden w-full relative">
          <motion.div
            initial={{ width: 0 }}
    ...
    <div className="px-4 py-3 bg-neutral-50/50 border-t border-slate-50 flex justify-between items-center mt-auto">
      <div className="flex flex-col">
        <span className="text-[6px] font-black text-neutral-400 uppercase tracking-widest">Presupuesto</span>
        <span className="text-[11px] font-black text-primary italic leading-none">{fmtQ(project.budget || 0)}</span>
      </div>
      <button
        onClick={(e) => onDelete(e, project.id)}
        aria-label="Eliminar proyecto"
        className="p-1.5 text-slate-300 hover:text-error hover:bg-(--color-error-bg) rounded-lg transition-all"
      >
        <Trash2 size={14} />
      </button>
    </div>
